...
```{r child="man/rmdchunks/_fundiversity_functions.Rmd", fig.cap="Table of available functions in fundiversity. Each column indicates which ones are parallelizable and which ones are memoizable."}
```
...
`fundiversity` only focuses on computing functional diversity indices, so the rest of the analysis has to be performed with other packages.
