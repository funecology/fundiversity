...
We want to emphasize here that `fundiversity` does not assume anything in the upstream of the computation of functional diversity indices. The trait standardization and the computation of a trait distance are to the user's discretion. They are here provided as a fully developed workflow example. The specific functions used in the previous chunk can vary depending on the scientific question, the nature of the traits, or the transformation needed. `fundiversity` does not provide any functions to deal with these upstream choices as it is the user's responsibility to carefully examine these choices.
