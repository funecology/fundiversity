...
The parallel computation are done by splitting across sites, so they may be interesting when facing a huge number of sites. However, given the efficiency of `fundiversity` functions, and the overhead costs associated with parallel computation we recommend using parallel computation only when working with matrices of at least 10.000 sites, or when hitting a performance limit of the default sequential execution.
