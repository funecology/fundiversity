...
fd_raoq(traits = NULL, dist_matrix = trait_dissim)

fd_raoq(traits = NULL, site_sp_birds, dist_matrix = as.matrix(trait_dissim))
```
**NB**: If you do provide a dissimilarity matrix through the `dist_matrix` argument, set up the `traits` argument to `NULL` explicitly with `traits = NULL`.
...
There are several other options available to standardize trait values, reviewed in Leps et al. [-@Leps_Quantifying_2006a].
